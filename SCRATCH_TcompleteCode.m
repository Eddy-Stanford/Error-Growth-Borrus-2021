...
q=1
for run = 2:3
var_diff(:,q,:,:,:,:) = Tmaster(:,1,:,:,:,:)-Tmaster(:,run,:,:,:,:);
q=q+1
end
%%
%90 lats = 88 bins
...
xline([0 -45 45],'--')
yline([0],'--')
xlabel('latitude'); ylabel('T from base');title('T and lat vs Sat Time T'); set(gca,'FontSize',14);

figure(2),clf,hold on
contourf(X,TempAxis,SatIntGrid',[17 19 21 23 25 27],'ShowText','on')
colorbar
axis([-80 80 -15 35])
caxis([17 25])
xline([0 -45 45],'--')
